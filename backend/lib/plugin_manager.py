...
class PluginManager():
    ...
    def __init__(self, plugin_dirs, registry=None):
    ...
    def load_all(self, check=True):
        for plugin_dir in self._plugin_dirs:
            if plugin_dir.startswith("./"):
                plugin_dir = plugin_dir[2:]
            module_list = os.listdir(plugin_dir)
            for module_dir in module_list:
                dir_list = os.listdir(os.path.join(plugin_dir, module_dir))
                if "plugin.py" in dir_list:
                    temp = importlib.import_module(f"{plugin_dir}.{module_dir}.plugin")
